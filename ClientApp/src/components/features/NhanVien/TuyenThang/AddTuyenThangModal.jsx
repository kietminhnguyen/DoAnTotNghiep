...
export class AddTuyenThangModal extends Component {
    ...
    handleSubmit(event) {
        ...
        var ngayHienTaiDOB = format(new Date(), 'yyyy-MM-dd')
        //2020-08-01 vs 1998-09-10 => 21 tuổi
        var tinhTuoi = differenceInYears(
            new Date(ngayHienTai),
            new Date(event.target.NhanvienNgaySinh.value)
        )

        //Nhập ngày cấp VS Nhập ngày sinh => phải hơn hơn bằng 14 tuổi mới đc cấp CMND
        var tinhNgayCapCMND = differenceInYears(
            new Date(event.target.NhanvienNgNgayCapCMMM.value),
            new Date(event.target.NhanvienNgaySinh.value)
        )//console.log(tinhNgayCapCMND)
        //Nếu ngày cấp CMND trước Ngày hiện tại => -1 (bằng là 0)
        var ktNgayCapVsNgayHienTai = compareAsc(new Date(event.target.NhanvienNgNgayCapCMMM.value),
            new Date(ngayHienTai)) //console.log(ktNgayCapVsNgayHienTai)

        var ktNgaySinhVsNgayHienTai = compareAsc(new Date(event.target.NhanvienNgaySinh.value),
            new Date(ngayHienTaiDOB)) //console.log(ktNgayCapVsNgayHienTai)

        //Kt trùng số CMND
        let flagSoCMND = false
        for (let i = 0; i < this.state.nhanviens.length; i++) {
            if (this.state.nhanviens[i].soCmnn == event.target.NhanvienSoCMNN.value) {
                flagSoCMND = true
            }
            else {
                flagSoCMND = false
        ...
        if (tinhTuoi < 18 || ktNgaySinhVsNgayHienTai == 1) { // chưa đủ tuổi đi làm
            flagTuoi = false
            //alert("Ngày sinh chưa hợp lệ. YÊU CẦU NHÂN VIÊN PHẢI TRÊN 18 Tuổi!!!")
            this.setState({ snackbaropen: true, snackbarmsg: "Ngày sinh chưa hợp lệ" });
        } else {
            if (tinhNgayCapCMND < 14 || ktNgayCapVsNgayHienTai == 1) { // chưa đủ 14 tuổi & ngày cấp sau hiện tại 
                ...
                this.setState({ snackbaropen: true, snackbarmsg: "Ngày cấp CMND chưa hợp lệ" });
            }
            else {
                if (flagSoCMND == true) {
                    //alert("Số CMND đã bị trùng")
                    this.setState({ snackbaropen: true, snackbarmsg: "Số CMND đã bị trùng" });
                }
                // else {
                ...
                //     }
                    ...
                    if ((flagTuoi == true && flagNgayCapCMND == true)
                        ...
                        axios.post('https://localhost:44390/api/nhanviens/', {
                            ...
                            trangthaiHdthuViec: null,
                            trangthaiHdchinhThuc: null,
                        ...
                        //alert('thanh cong');
                        this.setState({ snackbaropen: true, snackbarmsg: "VUI LÒNG KIỂM TRA LẠI!!!" });
                    }
                }
                //}
            }
    ...
    checkHideModal() {
        ...
        if (this.state.formErrors.NhanvienTEN == "." // không đỏ sẽ đóng
            && this.state.formErrors.NhanvienHO == "."
            && this.state.formErrors.NhanvienEmail == "."
            && this.state.formErrors.NhanvienSoCMNN == "."
            && this.state.formErrors.NhanvienSDT == "."
        ) {
    ...
    handleChangeValid = e => {
        ...
        switch (name) {
            case "NhanvienTEN":
                formErrors.NhanvienTEN = value.length < 1 || hoRegexp.test(value) ? "Tên chưa hợp lệ" : ".";
                break;
            case "NhanvienHO":
                formErrors.NhanvienHO = value.length < 2 || hoRegexp.test(value) ? "Họ đệm chưa hợp lệ" : ".";
                break;
            case "NhanvienEmail":
                formErrors.NhanvienEmail = emailRegex.test(value) ? "." : "Email chưa hợp lệ";
                break;
            case "NhanvienSoCMNN":
                formErrors.NhanvienSoCMNN = cmndRegex9.test(value) || cmndRegex12.test(value) ? "." : "Số CMND chưa hợp lệ";
                break;
            case "NhanvienSDT":
                formErrors.NhanvienSDT = sdtRegex.test(value) ? "." : "Số điện thoại chưa hợp lệ";
                break;
            default:
                break;
        }
    ...
    checkErrorHo() {
        if (this.state.formErrors.NhanvienHO != "." && this.state.formErrors.NhanvienHO != "*") {
            return true
        }
        return false
    }
    checkErrorTen() {
        if (this.state.formErrors.NhanvienTEN != "." && this.state.formErrors.NhanvienTEN != "*") {
            return true
        }
        return false
    }
    checkErrorEmail() {
        if (this.state.formErrors.NhanvienEmail != "." && this.state.formErrors.NhanvienEmail != "*") {
            return true
        }
        return false
    }
    checkErrorSDT() {
        if (this.state.formErrors.NhanvienSDT != "." && this.state.formErrors.NhanvienSDT != "*") {
            return true
        }
        return false
    }
    checkErrorSoCMND() {
        if (this.state.formErrors.NhanvienSoCMNN != "." && this.state.formErrors.NhanvienSoCMNN != "*") {
            return true
        }
        return false
    }
                    ...
                    <Col sm={12}>
                        <Row className="mt-1">
                            ...
                            <Col sm={3} className="mt-3">
                                ...
                                ></StyledKeID>
                                {formErrors.NhanvienHO.length > 0 && (
                                    <p><span className="errorMessage">{formErrors.NhanvienHO}</span></p>
                                )}
                            </Col>
                            <Col sm={2} className="mt-3">
                                <TextField
                                ...
                                {formErrors.NhanvienTEN.length > 0 && (
                                    <p><span className="errorMessage">{formErrors.NhanvienTEN}</span></p>
                                )}
                            </Col>
                            <Col sm={2} className="mt-3">
                                <StyledGioiTinh
                        ...
                        <Row className="mb-3">
                            <Col sm={3}>
                                <label className="btn btn-link btn-sm ml-5">
                                    CHỌN HÌNH<input type="file" hidden name="NhanvienHINH"
                    ...
                    <Col sm={12} className="ml-2">
                        <Row className="mt-4">
                            <Col sm={2} >
                                ...
                                />
                                {formErrors.NhanvienSDT.length > 0 && (
                                    <p><span className="errorMessage">{formErrors.NhanvienSDT}</span></p>
                                )}
                            </Col>
                            <Col sm={3} >
                                <StyledKeID
                                ...
                                ></StyledKeID>
                                {formErrors.NhanvienEmail.length > 0 && (
                                    <p><span className="errorMessage">{formErrors.NhanvienEmail}</span></p>
                                )}
                            </Col>
                            <Col sm={2} >
                                <StyledHonNhan
                        ...
                        <Row className="mt-2 mb-3">
                            ...
                            <Col sm={3} >
                                <StyledKeID
                                    ...
                                    select
                                    label="Chức vụ"
                                    defaultValue={this.layGiaTriMacDinhChucVu("Nhân viên")}
                                >
                                    {
